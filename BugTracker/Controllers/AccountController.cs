        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    //Build Email Message
                    MailMessage mailMessage = new MailMessage();
                    mailMessage.To.Add(new MailAddress(model.Email, model.Email));
                    mailMessage.From = new MailAddress(user.Email, "From");
                    mailMessage.Subject = "BuggyTracker - Resend your password";

                    string code = await UserManager.GeneratePasswordResetTokenAsync(user.Id);
                    var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);

                    string bodytext = String.Concat(user.Email, @"<p>Reset Password", "Please reset your password by clicking", "</p> <p><a href='"
                                   , callbackUrl, "'>Here</a></p>");
                    mailMessage.AlternateViews.Add(AlternateView.CreateAlternateViewFromString(bodytext, null, MediaTypeNames.Text.Html));
